  ...
  onSubmit(action: any): void {
    ...
    if (this.createAccommodationForm.valid) {
      ...
      let loader = this.loadingCtrl.create({
        ...
        content: 'Accommodation step added...'
     ...
     loader.present();
      ...
      // Tur adımlarının sırasının tutulduğu diziye eklenen adımın id'si ekleniyor
         self.compl.stepSequences.push({id: newAction.key, type: "ACCOMMODATION"});

      // Daha sonra tur tablosunda o an hangi turdaysak o turun adımlar kaydına ulaşıyoruz (id ile)
         let countRef = self.toursRef.child(tourId + '/steps/');

      // İlgili turun adımlar (steps) özelliğini step sıralarını tutan dizi ile güncelliyoruz
...
submit(action: any): void {
  ...
  startUploading(file) {
        ...
        //User id'sini firebase'den çekiyoruz

        let uid = self.dataService.getLoggedInUser().uid;
        let progress: number = 0;

       
        // Resim upload edilirken loader dönüyor
        let loader = this.loadingCtrl.create({
        ...
        // Resme üst bilgiler (ekstra bilgiler) ekleniyor
        var metadata = {
